...
int ParamMedicApplication::main(int argc, char** argv) {
  processFiles(Params::GetStrings("spectrum-file"));
  /*TODO if (precursorFailure.empty()) {
...
void ParamMedicApplication::processFiles(const vector<string>& files) {
  ...
  RunAttributeResult errorCalcResult = errorCalc.summarize();

  vector<Modification> mods;
  for (vector<RunAttributeDetector*>::const_iterator i = modDetectors.begin();
       i != modDetectors.end();
       i++) {
    RunAttributeResult result = (*i)->summarize();
    const vector<Modification>& resultMods = result.getMods();
    mods.insert(mods.end(), resultMods.begin(), resultMods.end());
  }

  carp(CARP_INFO, "");
  carp(CARP_INFO, "Search parameter recommendations:");
  string errorCalcMessagesAll = errorCalcResult.getValue("search_param_messages");
...
void ErrorCalc::processSpectrum(
  ...
  const vector<FLOAT_T>& binnedSpectrum
) {
...
RunAttributeResult ErrorCalc::summarize() const {
  ...
  result.setValue("search_param_messages", StringUtils::Join(messages, '\n'));
  result.setValue("precursor_sigma_ppm", StringUtils::ToString(precursorSigmaPpm));
  result.setValue("frag_sigma_ppm", StringUtils::ToString(fragmentSigmaPpm));
  result.setValue("precursor_prediction_ppm", StringUtils::ToString(precursorPredictionPpm));
  result.setValue("fragment_prediction_th", StringUtils::ToString(fragmentPredictionTh));
  return result;
}
...
vector<FLOAT_T> binSpectrum(const Spectrum* spectrum) {
  int numBins = (int)((BINNING_MAX_MZ - BINNING_MIN_MZ)/AVERAGINE_PEAK_SEPARATION) + 1;
  vector<Peak> peaks = spectrum->getPeaks();
  vector<FLOAT_T> matrix(numBins, 0);
  for (vector<Peak>::const_iterator i = peaks.begin(); i != peaks.end(); i++) {
    FLOAT_T mz = i->getLocation();
    FLOAT_T intensity = i->getIntensity();
  ...
  double sum = 0;
  for (vector<FLOAT_T>::const_iterator i = matrix.begin(); i != matrix.end(); i++) {
    sum += *i;
  }
  if (sum > 0) {
    for (vector<FLOAT_T>::iterator i = matrix.begin(); i != matrix.end(); i++) {
      *i = (FLOAT_T)((double)*i / sum);
    }
  }
  return matrix;
}
...
int processSpectra(const vector<string>& files, vector<RunAttributeDetector*> detectors) {
  ...
  for (vector<string>::const_iterator i = files.begin(); i != files.end(); i++) {
    ...
    for (SpectrumIterator j = collection->begin(); j != collection->end(); j++) {
      ...
      vector<FLOAT_T> binned = ParamMedic::binSpectrum(*j);
