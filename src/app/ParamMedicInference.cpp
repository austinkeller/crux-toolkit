...
RunAttributeResult Tmt6vs10Detector::summarize() const {
  ...
  for (map< int, vector<double> >::const_iterator i = nominalMassAllPeaks_.begin();
    ...
    if (curPeaks.size() >= TMT610_MINPEAKS_FOR_DETECT) {
      ...
      carp(CARP_DEBUG, "    %f-%f",
           min_element(curPeaks.begin(), curPeaks.end()),
           max_element(curPeaks.begin(), curPeaks.end()));
      ...
      int nPeaksNearPeak6 = bisect(curPeaks, maxMassPeak6) - bisect(curPeaks, minMassPeak6) + 1;
      int nPeaksNearPeak10 = bisect(curPeaks, maxMassPeak10) - bisect(curPeaks, minMassPeak10) + 1;
...
size_t Tmt6vs10Detector::bisect(const vector<double>& v, double x) const {
...
: foundMs3Scans_(false) {
  ...
  for (size_t i = 0; i < sizeof(types) / sizeof(REPORTER_ION_TYPE); i++) {
    ...
    case TMT_2PLEX:
      ...
      break;
    case TMT_6PLEX:
      mzs.push_back(128.1344);
      mzs.push_back(129.1378);
      mzs.push_back(130.1411);
      mzs.push_back(131.1382);
      break;
    case ITRAQ_4PLEX:
      mzs.push_back(114.0);
      mzs.push_back(115.0);
...
void SilacDetector::processSpectrum(
  ...
  double mass = calcMH(spectrum->getPrecursorMz(), zStates.front().getCharge());
...
RunAttributeResult SilacDetector::summarize() const {
  ...
  if (separations.size() < modBinDistances_.size() + controlBinDistances_.size()) {
    carp(CARP_WARNING, "A specified separation is also a control separation! Specified: %s",
         StringUtils::Join(modBinDistances_, ','));
  }
...
void EnzymeDetector::processSpectrum(
  ...
  for (map<char, double>::const_iterator i = aaMasses_.begin(); i != aaMasses_.end(); i++) {
    ...
    const char aa = i->first;
    FLOAT_T binY1 = binnedSpectrum[aaY1Charge1Bin_[aa]];
